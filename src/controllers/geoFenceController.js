// const axios = require('axios');
// const GeoFence = require('../models/GeoFence');
// const { getToken } = require("../services/gpsTokenManager");
// const Vehicle = require('../models/Vehicle');
// const { getVehicleLocation } = require('../controllers/vehicleController');

// exports.createGeoFence = async (req, res) => {
//   console.log('🔵 createGeoFence API HIT');
//   console.log('📥 Incoming Body:', req.body);

//   try {
//     const token = await getToken();

//     console.log('🔐 Token:', token);

//     const { imei, fenceName, radius, lat, lng } = req.body;

//     if (!imei || !lat || !lng || !radius) {
//       console.log('❌ Missing fields');
//       return res.status(400).json({
//         success: false,
//         message: 'Missing required fields',
//       });
//     }

//     const existingFences = await GeoFence.find({ imei });
//     console.log('🔍 Existing Fences Count:', existingFences.length);

//     for (const fence of existingFences) {
//       if (!fence.fenceId) continue;

//       try {
//         console.log('🗑️ Deleting fence:', fence.fenceId);

//         const deleteRes = await axios.delete(
//           `https://open.iopgps.com/api/fence/del/${fence.fenceId}`,
//           {
//             params: { imei },
//             headers: {
//               accessToken: token,
//             },
//           }
//         );

//         console.log('📥 Delete Response:', deleteRes.data);

//       } catch (err) {
//         const status = err.response?.status;

//         if (status === 404) {
//           console.log(`⚠️ Fence ${fence.fenceId} already deleted`);
//         } else {
//           console.log(
//             `❌ Failed deleting ${fence.fenceId}:`,
//             err.response?.data || err.message
//           );
//         }
//       }
//     }

//     if (existingFences.length > 0) {
//       await GeoFence.deleteMany({ imei });
//       console.log('✅ Cleared all old fences from DB');
//     }

//     const payload = {
//       triggerType: 1,
//       mapType: 1,
//       oneTime: 1,
//       type: 2,
//       fenceName,
//       imei,
//       setting: `${lng} ${lat} ${radius}`,
//     };

//     console.log('📦 Creating new fence payload:', payload);

//     const response = await axios.post(
//       'https://open.iopgps.com/api/fence/add',
//       payload,
//       {
//         headers: {
//           'Content-Type': 'application/json',
//           accessToken: token,
//         },
//       }
//     );

//     console.log('📥 Create Response:', response.data);

//     if (response.data?.resultBean?.code !== 0) {
//       console.log('❌ Fence creation failed:', response.data);

//       return res.status(400).json({
//         success: false,
//         message: 'Failed to create fence',
//         data: response.data,
//       });
//     }

//     const fenceId = response.data?.fenceId;
//     console.log('🆔 New Fence ID:', fenceId);

//     const savedFence = await GeoFence.create({
//       imei,
//       fenceName,
//       radius,
//       lat,
//       lng,
//       fenceId,
//     });

//     console.log('💾 Saved Fence:', savedFence);

//     return res.json({
//       success: true,
//       message: 'Geo-fence replaced successfully',
//       data: savedFence,
//     });

//   } catch (error) {
//     console.log(
//       '🔥 createGeoFence ERROR:',
//       error.response?.data || error.message
//     );

//     return res.status(500).json({
//       success: false,
//       error: error.response?.data || error.message,
//     });
//   }
// };

// exports.getGeoFences = async (req, res) => {
//   console.log('🔵 getGeoFences API HIT');

//   try {
//     const { imei } = req.params;
//     console.log('📥 IMEI:', imei);

//     if (!imei) {
//       console.log('❌ IMEI missing');
//       return res.status(400).json({
//         success: false,
//         message: 'IMEI is required',
//       });
//     }

//     const fences = await GeoFence.find({ imei }).sort({ createdAt: -1 });

//     console.log('📤 Fetched Fences:', fences.length);

//     res.json({
//       success: true,
//       data: fences,
//     });

//   } catch (error) {
//     console.log('🔥 Fetch Fence Error:', error.message);

//     res.status(500).json({
//       success: false,
//       message: 'Failed to fetch geo-fences',
//     });
//   }
// };

// exports.enableAutoParkAndGeofence = async (req, res) => {
//   try {
//     const { userId, enable } = req.body;
//     console.log('🚀 Auto Secure DB Update API HIT');

//     if (!userId) {
//       return res.status(400).json({ success: false, message: 'userId required' });
//     }

//     await Vehicle.updateMany(
//       { userId },
//       { $set: { autoPark: enable } }
//     );

//     console.log(`⚙️ autoPark set to ${enable} for all vehicles belonging to ${userId}`);


//     return res.json({
//       success: true,
//       message: `AutoPark flag ${enable ? 'enabled' : 'disabled'} in database`,
//     });

//   } catch (error) {
//     console.log('🔥 ERROR:', error.message);
//     return res.status(500).json({ success: false, message: error.message });
//   }
// };

// exports.deleteAllFences = async (req, res) => {
//   try {
//     await GeoFence.deleteMany({});
//     console.log("🗑️ All fences deleted");

//     res.json({
//       success: true,
//       message: "All geofences deleted"
//     });
//   } catch (err) {
//     console.log("❌ Error:", err.message);
//     res.status(500).json({ success: false });
//   }
// };


const axios = require('axios');
const GeoFence = require('../models/GeoFence');
const { getToken } = require('../services/gpsTokenManager');
const Vehicle = require('../models/Vehicle');

const VEHICLE_SETTINGS_KEYS = [
  'autoSecureWhenParked',
  'geoFenceBreachProtection',
  'autoEngineCutOffGeo',
  'notifyOnGeoFenceBreach',
  'trackerDisconnectionProtection',
  'autoEngineCutOffOnTrackerDisconnect',
  'notifyOnTrackerDisconnect',
  'ignitionOffEnabled',
  'ignitionOnEnabled',
];

exports.updateVehicleSettings = async (req, res) => {
  console.log('🔵 updateVehicleSettings API HIT');
  console.log('📥 Body:', req.body);

  try {
    const { imei, settings } = req.body;

    if (!imei) {
      return res.status(400).json({ success: false, message: 'imei is required' });
    }

    if (!settings || typeof settings !== 'object') {
      return res.status(400).json({ success: false, message: 'settings object is required' });
    }

    const $set = {};
    for (const key of VEHICLE_SETTINGS_KEYS) {
      if (key in settings) {
        $set[key] = Boolean(settings[key]);
      }
    }

    if (Object.keys($set).length === 0) {
      return res.status(400).json({ success: false, message: 'No valid settings fields provided' });
    }

    console.log('⚙️ Applying settings to vehicle:', imei, $set);

    const updated = await Vehicle.findOneAndUpdate(
      { imei },
      { $set },
      { new: true, select: VEHICLE_SETTINGS_KEYS.join(' ') + ' imei vehicleNickname' }
    );

    if (!updated) {
      return res.status(404).json({ success: false, message: 'Vehicle not found' });
    }

    console.log('✅ Vehicle settings updated:', updated);

    return res.json({
      success: true,
      message: 'Vehicle settings updated successfully',
      data: updated,
    });
  } catch (error) {
    console.error('🔥 updateVehicleSettings ERROR:', error.message);
    return res.status(500).json({ success: false, message: error.message });
  }
};


exports.getVehicleSettings = async (req, res) => {
  console.log('🔵 getVehicleSettings API HIT');

  try {
    const { imei } = req.params;

    if (!imei) {
      return res.status(400).json({ success: false, message: 'imei is required' });
    }

    const vehicle = await Vehicle.findOne(
      { imei },
      VEHICLE_SETTINGS_KEYS.join(' ') + ' imei vehicleNickname'
    );

    if (!vehicle) {
      return res.status(404).json({ success: false, message: 'Vehicle not found' });
    }

    console.log('📤 Settings fetched for:', imei);

    return res.json({
      success: true,
      data: vehicle,
    });
  } catch (error) {
    console.error('🔥 getVehicleSettings ERROR:', error.message);
    return res.status(500).json({ success: false, message: error.message });
  }
};


exports.createGeoFence = async (req, res) => {
  console.log('🔵 createGeoFence API HIT');
  console.log('📥 Incoming Body:', req.body);

  try {
    const token = await getToken();
    const { imei, fenceName, radius, lat, lng } = req.body;

    if (!imei || !lat || !lng || !radius) {
      return res.status(400).json({ success: false, message: 'Missing required fields' });
    }

    const existingFences = await GeoFence.find({ imei });
    console.log('🔍 Existing Fences:', existingFences.length);

    for (const fence of existingFences) {
      if (!fence.fenceId) continue;
      try {
        await axios.delete(`https://open.iopgps.com/api/fence/del/${fence.fenceId}`, {
          params: { imei },
          headers: { accessToken: token },
        });
        console.log('🗑️ Deleted fence:', fence.fenceId);
      } catch (err) {
        if (err.response?.status === 404) {
          console.log(`⚠️ Fence ${fence.fenceId} already deleted`);
        } else {
          console.log(`❌ Failed deleting ${fence.fenceId}:`, err.response?.data || err.message);
        }
      }
    }

    if (existingFences.length > 0) {
      await GeoFence.deleteMany({ imei });
      console.log('✅ Cleared old fences from DB');
    }

    const payload = {
      triggerType: 1,
      mapType: 1,
      oneTime: 1,
      type: 2,
      fenceName,
      imei,
      setting: `${lng} ${lat} ${radius}`,
    };

    const response = await axios.post('https://open.iopgps.com/api/fence/add', payload, {
      headers: { 'Content-Type': 'application/json', accessToken: token },
    });

    console.log('📥 Create Response:', response.data);

    if (response.data?.resultBean?.code !== 0) {
      return res.status(400).json({ success: false, message: 'Failed to create fence', data: response.data });
    }

    const fenceId = response.data?.fenceId;
    const savedFence = await GeoFence.create({ imei, fenceName, radius, lat, lng, fenceId });

    return res.json({ success: true, message: 'Geo-fence replaced successfully', data: savedFence });
  } catch (error) {
    console.error('🔥 createGeoFence ERROR:', error.response?.data || error.message);
    return res.status(500).json({ success: false, error: error.response?.data || error.message });
  }
};

exports.getGeoFences = async (req, res) => {
  try {
    const { imei } = req.params;
    if (!imei) return res.status(400).json({ success: false, message: 'IMEI is required' });

    const fences = await GeoFence.find({ imei }).sort({ createdAt: -1 });
    res.json({ success: true, data: fences });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Failed to fetch geo-fences' });
  }
};

exports.enableAutoParkAndGeofence = async (req, res) => {
  try {
    const { userId, enable } = req.body;
    if (!userId) return res.status(400).json({ success: false, message: 'userId required' });

    await Vehicle.updateMany({ userId }, { $set: { autoPark: enable } });
    console.log(`⚙️ autoPark set to ${enable} for all vehicles of user: ${userId}`);

    return res.json({ success: true, message: `AutoPark ${enable ? 'enabled' : 'disabled'}` });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
};

exports.deleteAllFences = async (req, res) => {
  try {
    await GeoFence.deleteMany({});
    res.json({ success: true, message: 'All geofences deleted' });
  } catch (err) {
    res.status(500).json({ success: false });
  }
};